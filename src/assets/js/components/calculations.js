...
function bodyMassIndex(weight, height) {
	...
	if (meter && height) {
		let numberBodyMassIndex = +weight / (+meter * +meter);

		updateBMI(numberBodyMassIndex);
		if (inputFormIMT) {
			inputFormIMT.value = numberBodyMassIndex.toFixed(2);
		}
	}
}
...
function recommendedAmountWater(weight) {
	...
	if (htmlBlock) {
		htmlBlock.innerHTML = `${volumeWater}`;
	}
	if (inputWater) {
...
/**
 *  отоброжения индекса тела
 * @param {number} [bmi] -индекс тела
 */
